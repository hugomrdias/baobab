...
function cloner(deep, item) {
  if (!item || typeof item !== 'object' || item instanceof Error || item instanceof MonkeyDefinition || item instanceof Monkey || typeof ArrayBuffer === 'function' && item instanceof ArrayBuffer) return item;
...
function freezer(deep, o) {
  if (typeof o !== 'object' || o === null || o instanceof Monkey) return;
  ...
  if (Array.isArray(o)) {
    ...
    for (k in o) {
      if (type.lazyGetter(o, k)) continue;
...
export function getIn(object, path) {
  ...
  for (i = 0, l = path.length; i < l; i++) {
    ...
    if (typeof path[i] === 'function') {
      if (!type.array(c)) return NOT_FOUND_OBJECT;
...
function merger(deep) {
  ...
  for (i = 1, l = objects.length; i < l; i++) {
    ...
    for (k in t) {
      if (deep && type.object(t[k]) && !(t[k] instanceof Monkey)) {
 ...
 */
export function splice(array, startIndex, nb) {
  ...
  // Solving startIndex
  if (type.function(startIndex)) startIndex = index(array, startIndex);
  if (type.object(startIndex)) startIndex = index(array, function (e) {
