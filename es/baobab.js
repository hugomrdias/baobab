...
var Baobab = function (_Emitter) {
  ...
  function Baobab(initialData, opts) {
    ...
    if (!type.object(initialData) && !type.array(initialData)) throw makeError('Baobab: invalid data.', { data: initialData });
  ...
  Baobab.prototype._refreshMonkeys = function _refreshMonkeys(node, path, operation) {
    ...
    var clean = function clean(data) {
      ...
      if (data instanceof Monkey) {
        data.release();
        _update(_this2._monkeys, p, { type: 'unset' }, {
          immutable: false,
          persistent: false,
          pure: false
        });
    ...
    var walk = function walk(data) {
      ...
      if (data instanceof MonkeyDefinition || data instanceof Monkey) {
        var monkeyInstance = new Monkey(_this2, p, data instanceof Monkey ? data.definition : data);

        _update(_this2._monkeys, p, { type: 'set', value: monkeyInstance }, {
          immutable: false,
          persistent: false,
          pure: false
        });
  ...
  Baobab.prototype.select = function select(path) {
    ...
    if (!type.path(path)) throw makeError('Baobab.select: invalid path.', { path: path });
  ...
  Baobab.prototype.update = function update(path, operation) {
    ...
    if (!type.operationType(operation.type)) throw makeError('Baobab.update: unknown operation type "' + operation.type + '".', { operation: operation });
    ...
    // Read-only path?
    var monkeyPath = type.monkeyPath(this._monkeys, solvedPath);
    ...
    var result = _update(this._data, solvedPath, realOperation, this.options);
...
export var monkey = function monkey() {
  ...
  if (args.length === 1 && typeof args[0] !== 'function') return new MonkeyDefinition(args[0]);

  return new MonkeyDefinition(args);
};

export { monkey as dynamicNode, Cursor, MonkeyDefinition, Monkey, type, helpers };
